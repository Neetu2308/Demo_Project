...
- Name: logout.spec.js >> Logout
- Location: tests\logout.spec.js:5:5

# Error details

```
ReferenceError: validcreds is not defined
```
...
```ts
  ...
  4  | import {Logout} from '../pages/Logout'
  5  | test ('Logout', async({page})=>
  6  | {
  7  |     const loginpage=new LoginPageDemo(page)
  8  |     await loginpage.goto()
> 9  |      await loginpage.login_action(validcreds.username,validcreds.password)
     |                                   ^ ReferenceError: validcreds is not defined
  10 |     await expect(page.locator('#nameofuser')) .toContainText('Welcome neetus')
  11 |     await loginpage.logout()
  12 |     await expect(page.locator('#login2')).toHaveText('Log in')
  13 | 
  14 | })
  15 |   
```